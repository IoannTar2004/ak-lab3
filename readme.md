...
- `movh` - перенести младший байт[7:0] в старший байт[31:24] аккумулятора. Используется для подготовки данных к передаче по spi.
...
- Каждый порт имеет свой адрес.
- Данные (сигналы) на портах принимают значения 0 или 1. Инвертирование сигнала на порте происходит в методе
```signal```.
...
программы у нас нет полученных символов, которые процессор мог бы отправить на Slave (относится к cat). Но в любом 
случае, отправить данные нужно, даже если их нет. Поэтому во время первой передачи отправляется единица, чтобы 
лишние данные не добавились в буфер вывода и не испортили вывод. Проверка осуществляется в методе ```add_to_output_buffer```
